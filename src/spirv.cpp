...
void buildCryptonightR(const struct V4_Instruction* code, bool hi, bool light, int localSize, bool randomMath64, uint32_t iterations, uint32_t mask,CryptoType cryptoType ) {
	...
	s << "layout(binding = 0) buffer scratchpadsBuffer1 { uvec4 scratchpad1[]; };layout(binding = 1) buffer scratchpadsBuffer2 { uvec4 scratchpad2[]; };layout(binding = 2) buffer statesBuffer { uint64_t states[]; };layout(binding = 4) buffer Params { uint64_t target;uint64_t global_work_offset;uint memorySize;uint iterations;uint mask;uint threads;uint scratchpadSplit;uint memchunk2;};layout(binding = 5) buffer constants{uint AES0_C[256];};layout(binding = 6) buffer inputsBuffer {uint64_t inputs[];};\n";
	s << "layout (local_size_x = " << localSize << ", local_size_y = 1) in;\n";
	s << "shared uint AES0[256], AES1[256], AES2[256], AES3[256];\n";
	s << "#define ALN(x) (((x) & (memChunk-1)) + ((x) >> _memchunk2) * 16 * memChunk)\n";
	s << "#define INDEX0  (scratchpadOffset + ALN((idx0 >> 4)))\n";
	s << "#define INDEX1  (scratchpadOffset + ALN((idx0 >> 4) ^ 1))\n";
	s << "#define INDEX2  (scratchpadOffset + ALN((idx0 >> 4) ^ 2))\n";
	s << "#define INDEX3  (scratchpadOffset + ALN((idx0 >> 4) ^ 3))\n";
	...
	s << "uint getScrathpadOffset(uint gIdx) {	return (gl_GlobalInvocationID.x >> 4) * (memorySize >> 4) * 16 + (1<<memchunk2) * (gl_GlobalInvocationID.x & 15);}\n";
	s << "void main() {\n";
	...
	s << "const uint _memchunk2 = memchunk2;\n";
	s << "const uint memChunk = 1 << memchunk2;\n";
	s << "const uint scratchpadOffset = getScrathpadOffset(gIdx);\n";
	...
	s << "_chunk0.x ^= AES0[BYTE(chunk0.x, 0)] ^ AES1[BYTE(chunk0.y, 1)] ^ AES2[BYTE(chunk0.z, 2)] ^ AES3[BYTE(chunk0.w, 3)]; \n";
	s << "_chunk0.y ^= AES0[BYTE(chunk0.y, 0)] ^ AES1[BYTE(chunk0.z, 1)] ^ AES2[BYTE(chunk0.w, 2)] ^ AES3[BYTE(chunk0.x, 3)]; \n";
	s << "_chunk0.z ^= AES0[BYTE(chunk0.z, 0)] ^ AES1[BYTE(chunk0.w, 1)] ^ AES2[BYTE(chunk0.x, 2)] ^ AES3[BYTE(chunk0.y, 3)]; \n";
	s << "_chunk0.w ^= AES0[BYTE(chunk0.w, 0)] ^ AES1[BYTE(chunk0.x, 1)] ^ AES2[BYTE(chunk0.y, 2)] ^ AES3[BYTE(chunk0.z, 3)]; \n";
